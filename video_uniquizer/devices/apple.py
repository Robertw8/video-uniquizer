"""Apple iPhone processing presets."""

from .base import DevicePreset

STANDARD_FPS = (23.976, 24.0, 25.0, 29.97, 30.0, 50.0, 59.94, 60.0)


def _iphone(
    model: str,
    *,
    preferred_fps: tuple[float, ...],
    jpeg_quality_range: tuple[int, int],
    sharpness_range: tuple[float, float],
) -> DevicePreset:
    return DevicePreset(
        manufacturer="Apple",
        model=model,
        platform="iOS",
        family="iPhone",
        allowed_fps=STANDARD_FPS,
        preferred_fps=preferred_fps,
        video_codec="h264",
        pixel_format="yuv420p",
        crf_range=(22, 25),
        speed_range=(0.985, 1.025),
        zoom_range=(0, 2),
        brightness_range=(-0.025, 0.025),
        contrast_range=(0.97, 1.04),
        saturation_range=(0.96, 1.04),
        sharpness_range=sharpness_range,
        noise_levels=(1, 2),
        jpeg_quality_range=jpeg_quality_range,
        image_zoom_range=(0, 2),
        image_brightness_range=(0.97, 1.03),
        image_contrast_range=(0.97, 1.04),
        image_saturation_range=(0.96, 1.04),
        image_sharpness_range=sharpness_range,
        image_noise_levels=(1, 2),
        make="Apple",
        metadata_family="QuickTime/EXIF",
    )


APPLE_PRESETS: tuple[DevicePreset, ...] = (
    _iphone(
        "iPhone 12",
        preferred_fps=(29.97, 30.0, 59.94),
        jpeg_quality_range=(90, 95),
        sharpness_range=(0.25, 0.48),
    ),
    _iphone(
        "iPhone 13",
        preferred_fps=(29.97, 30.0, 59.94),
        jpeg_quality_range=(91, 96),
        sharpness_range=(0.27, 0.50),
    ),
    _iphone(
        "iPhone 13 Pro",
        preferred_fps=(24.0, 29.97, 30.0, 59.94),
        jpeg_quality_range=(91, 96),
        sharpness_range=(0.28, 0.52),
    ),
    _iphone(
        "iPhone 14",
        preferred_fps=(29.97, 30.0, 59.94),
        jpeg_quality_range=(91, 96),
        sharpness_range=(0.28, 0.52),
    ),
    _iphone(
        "iPhone 14 Pro",
        preferred_fps=(24.0, 29.97, 30.0, 59.94),
        jpeg_quality_range=(91, 96),
        sharpness_range=(0.29, 0.53),
    ),
    _iphone(
        "iPhone 15",
        preferred_fps=(29.97, 30.0, 59.94),
        jpeg_quality_range=(92, 97),
        sharpness_range=(0.30, 0.54),
    ),
    _iphone(
        "iPhone 15 Pro",
        preferred_fps=(24.0, 29.97, 30.0, 59.94),
        jpeg_quality_range=(92, 97),
        sharpness_range=(0.30, 0.55),
    ),
)
